...
from bridge_dataset.auto_caption import auto_caption
...
def _generate_examples(paths) -> Iterator[Tuple[str, Any]]:
    ...
    def _parse_examples(episode_path):
        ...
        for k, example in enumerate(data):
            ...
            # don't use T5 for now
            language_embedding = np.zeros(512, dtype=np.float32)

            action_captions = auto_caption(example)
            ...
            for i in range(len(example["observations"])):
                ...
                for image_idx in range(4):
                    ...
                    if orig_key in example["observations"][i]:
                        ...
                        observation[new_key] = np.zeros_like(
                            example["observations"][i]["images0"]
                        )
                ...
                episode.append(
                    {
                        ...
                        "language_instruction": action_captions[i] + instruction,
                        "language_embedding": language_embedding,
                    }
                )
                # print(action_captions[i] + instruction)
            ...
            for image_idx in range(4):
                orig_key = f"images{image_idx}"
                new_key = f"image_{image_idx}"
                sample["episode_metadata"][f"has_{new_key}"] = (
                    orig_key in example["observations"]
                )
...
class BridgeDataset(MultiThreadedDatasetBuilder):
    ...
    MAX_PATHS_IN_MEMORY = (
        5  # number of paths converted & stored in memory before writing to disk
    )
                    ...
                    "episode_metadata": tfds.features.FeaturesDict(
                        {
                            ...
                            "episode_id": tfds.features.Scalar(
                                dtype=np.int32, doc="ID of episode in file_path."
                            ),
    ...
    def _split_paths(self):
        ...
        # base_paths = ["/home/andre/bridge_rlds_builder/bridge_dataset/data"]
        train_filenames, val_filenames = [], []
        for path in base_paths:
            for filename in glob.glob(f"{path}/**/*.npy", recursive=True):
                # TODO remove this
                # if len(train_filenames) > 100 or len(val_filenames) > 100:
                #     continue
